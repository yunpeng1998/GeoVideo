...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def copy_proj_layer(self, parent_model=None):

        device = self.proj.weight.device
        dtype = self.proj.weight.dtype
        in_features = self.proj.in_features
        out_features = self.proj.out_features


        new_linear = nn.Linear(in_features * 2, out_features, bias=True).to(dtype)
        ...
        with torch.no_grad():
            ...
            new_weight = torch.zeros((out_features, in_features * 2), dtype=dtype, device=device)


            new_weight[:, :in_features] = original_weight


            new_linear.weight.copy_(new_weight)


            new_linear.bias.copy_(self.proj.bias)
        with torch.no_grad():

            new_linear.weight[:, :in_features].requires_grad = True
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor):
        ...
        if self.patch_size_t is None:
            if getattr(self, 'proj_replaced', False):
                ...
                image_embeds, depth_embeds =  torch.chunk(image_embeds, 2, dim=2)
                image_embeds = image_embeds.reshape(-1, channels, height, width)
                depth_embeds = depth_embeds.reshape(-1, channels, height, width)
                image_embeds = torch.cat([image_embeds, depth_embeds], dim=2)
            ...
            if getattr(self, 'proj_replaced', False):
                ...
                depth_embeds = depth_embeds.permute(0, 1, 3, 4, 2)
                image_embeds = image_embeds.reshape(
                    batch_size, num_frames // p_t, p_t, height // p, p, width // p, p, channels
                )
                depth_embeds = depth_embeds.reshape(
                    batch_size, num_frames // p_t, p_t, height // p, p, width // p, p, channels
                )
                image_embeds = image_embeds.permute(0, 1, 3, 5, 7, 2, 4, 6).flatten(4, 7).flatten(1, 3)
                depth_embeds = depth_embeds.permute(0, 1, 3, 5, 7, 2, 4, 6).flatten(4, 7).flatten(1, 3)
                image_embeds = torch.cat([image_embeds, depth_embeds], dim=-1)
                image_embeds = self.proj(image_embeds)
            else:
                image_embeds = image_embeds.permute(0, 1, 3, 4, 2)
                image_embeds = image_embeds.reshape(
